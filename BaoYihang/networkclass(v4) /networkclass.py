'''
    python download method vison 1.4
    For Ftp download and http download
    Author : BaoYihang
    
    compare to the former vision:
        1.Turn the Download class into a decoration class
'''
...
class Download(object):
    ...
    def __call__(self, func, *args , **kwargs):
        if self.ope == 2:
            self.login("ouyangpeng", "ouyangpeng")
            # download single file
            self.download_file("G:/ftp_test/XTCLauncher.apk", "/App/AutoUpload/ouyangpeng/I12/Release/XTCLauncher.apk")    
            # download the whole dictionary
            self.download_file_tree("G:/ftp_test/", "App/AutoUpload/ouyangpeng/I12/")        
            self.close()
        else:
